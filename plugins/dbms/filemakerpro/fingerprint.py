...
from lib.core.settings import FILEMAKERPRO_ALIASES
from lib.request import inject
from plugins.generic.fingerprint import Fingerprint as GenericFingerprint

class Fingerprint(GenericFingerprint):
    def __init__(self):
        GenericFingerprint.__init__(self, DBMS.FILEMAKERPRO)
    ...
    def getFingerprint(self):?
        ...
        if kb.bannerFp:
            ...
            if release and version and servicepack:
                banVer = "%s %s " % (DBMS.FILEMAKERPRO, release)
    ...
    def checkDbms(self):
        if not conf.extensiveFp and Backend.isDbmsWithin(FILEMAKERPRO_ALIASES):
            setDbms("%s %s" % (DBMS.FILEMAKERPRO, Backend.getVersion()))
        ...
        infoMsg = "testing %s" % DBMS.FILEMAKERPRO
        logger.info(infoMsg)
        ...
        if result:
            infoMsg = "confirming %s" % DBMS.FILEMAKERPRO
            logger.info(infoMsg)
            ...
            warnMsg = "the back-end DBMS is not %s" % DBMS.FILEMAKERPRO
            logger.warn(warnMsg)
